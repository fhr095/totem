...
export default function SceneScreen({
  ...
  const [canCallAction, setCanCallAction] = useState(true); // Novo estado para controlar o timeout
  ...
  useEffect(() => {
    ...
    // Adicionar evento de clique com botão direito do mouse
    const handleRightClick = (event) => {
      event.preventDefault();
      setIdentify(prevState => !prevState); // Alterna o estado isIdentify
    };
  ...
  useEffect(() => {
    let timeout;
    if (isKioskMode && isIdentify && transcript === "" && canCallAction) {
      setCanCallAction(false);
      timeout = setTimeout(() => setCanCallAction(true), 3000); // Timeout de 5 segundos
    }
    return () => clearTimeout(timeout);
  }, [isKioskMode, isIdentify, transcript, canCallAction]);
  ...
  useEffect(() => {
    if (transcript !== "" && showQuestionAndResponse) {
  ...
  }, [transcript, showQuestionAndResponse]);
